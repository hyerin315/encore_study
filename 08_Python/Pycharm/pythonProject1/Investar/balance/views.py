...
def get_data(symbol):
    url = 'http://finance.naver.com/item/sise.nhn?code={}'.format(symbol) # 네이버 주식 -> 종목코드로 접근
                                                                          # ex) http://127.0.0.1:8000/balance/?035420=30&009270=100
                                                                          # 종목 하나 더 추가시엔 위처럼 '&'로 연결
...
def main_view(request):
    ...
    return render(request, 'balance.html', values)  # ⑪ values : 딕셔너리 형태
