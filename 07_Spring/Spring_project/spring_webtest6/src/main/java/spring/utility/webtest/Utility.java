...
public class Utility {
  ...
  public static String saveFileSpring(MultipartFile mf, String basePath) {
	    ...
	    String originalFilename = mf.getOriginalFilename(); //mf 파일이 사용자에게 선택되어지기 전에 뽑아지는 파일명 (경로가 없는 파일명)
	    try {
	      if (filesize > 0) { // 파일이 존재한다면
	        ...
	        if (oldfile.exists()) {
	          for (int k = 0; true; k++) {
	            ...
	            // 파일명으로 조합
	            oldfile = new File(basePath, "(" + k + ")" + originalFilename); //a.txt가 있으면 (0)a.txt를 만들어주겠다
	        ...
	        // 버퍼를 만든다.
	        int readBytes = 0;
	        byte[] buffer = new byte[8192]; //넉넉한 크기의 바이트형 배열
	 
	        while ((readBytes = inputStream.read(buffer, 0, 8192)) != -1) {// true일 경우 반복
