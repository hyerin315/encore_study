...
public class BbsDAO {
  ...
  public BbsDTO read(int bbsno) {
    ...
    try {
      ...
      if (rs.next()) { //boolean type을 next가 리턴해줌 (데이터가 있으면 true / 없으면 false)
        dto = new BbsDTO();
        dto.setBbsno(rs.getInt("bbsno")); //get으로 가져와서 set으로 담음
...
//  }
  ...
  public boolean create(BbsDTO dto) {
    ...
    sql.append(" VALUES( ");
    sql.append(" (SELECT NVL(MAX(bbsno), 0)+1 FROM bbs),");
    //* 오라클에서 자주 쓰는 NVL 함수 : 데이터가 하나도 없는 상태에서 맥스값을 리턴하면 null을 리턴함 
    //                           - 그래서 먼저, ,(콤마) 왼쪽의 값이 0이면 오른쪽의 값을 변환해서 가져와라 
    //                           (+1해서 1값으로 가져와라, 그리고 그 다음부터는 max 값에 +1)
    sql.append(" ?, ?, ?, ?, sysdate, ");
