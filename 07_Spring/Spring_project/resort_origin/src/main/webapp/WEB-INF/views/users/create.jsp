...
<script type="text/javascript">
  ...
  //3. 중복확인 버튼에 대한 클릭이벤트 핸들러 - 클릭하면 자동 호출
  // jQuery ajax 요청
  function checkID() {
    // $('#btn_close').attr("data-focus", "이동할 태그 지정");
    //input 태그 : 모두 val 값으로 가져옴
    ...
    if ($.trim(id).length == 0) { // id를 입력받지 않은 경우 (빈공간인 경우)
      msg = 'ID를 입력하세요.<br>ID 입력은 필수 입니다.<br>ID는 3자이상 권장합니다.';//권장일뿐 따로 if 처리는 하지 않았음
      ...
      $.ajax({
        url: './checkID', // spring execute, Controller Get 방식으로 요청
        type: 'get',  // post
        cache: false, // 응답 결과 임시 저장 취소
        async: true,  // true : 비동기 통신 (Ajax는 기본이 비동기 통신)
...
<body>
      ...
      <div class="modal-content">
        ...
        <div class="modal-footer">
          <button type="button" id="btn_close" data-focus="" class="btn btn-default" 
                      data-dismiss="modal">닫기</button><!-- 5. 중복확인 모달에 대한 닫기 버튼 -->
        </div>
      </div>
    </div>
  </div>
  ...
  <FORM name='frm' id='frm' method='POST' action='./create' class="form-horizontal" onsubmit="return send()">
    ...
    <div class="form-group">
      <label for="id" class="col-md-2 control-label" style='font-size: 0.9em;'>아이디*</label><!-- *표시 ㅣ not null (꼭 입력해줘야함), required 사용 -->
      <div class="col-md-10">
        ...
        <button type='button' id="btn_checkID" class="btn btn-info btn-md">중복확인</button><!-- 1. 중복확인 -->
...
<script>
    ...
    //4. 중복확인 버튼에 대한 클릭이벤트 핸들러 - 클릭하면 자동 호출
...
<!-- ---------- DAUM 우편번호 API 종료 ---------- -->
    ...
    <div class="form-group">
      <div class="col-md-offset-2 col-md-10">
        <button id='btn_send' class="btn btn-primary btn-md">가입</button><!--  타입 설정이 없으면 기본이 submit -->
