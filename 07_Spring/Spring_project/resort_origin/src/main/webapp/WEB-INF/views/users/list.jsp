...
<body>
  ...
  <TR>
    ...
    <TD class='td'><A href="./read?usersno=${usersno}">${usersVO.id}</A></TD>
    <TD class='td'><A href="./read?usersno=${usersno}">${usersVO.name}</A></TD>
    <TD class='td'>${usersVO.tel}</TD>
    <TD class='td'>
      <c:choose>
        <c:when test="${usersVO.address1.length() > 15 }"> <!-- 긴 주소 처리 -->
          ${usersVO.address1.substring(0, 15) }...
        </c:when>
        <c:otherwise>
          ${usersVO.address1}
        </c:otherwise>
    ...
    <TD class='td'>${usersVO.rdate.substring(0, 10)}</TD> <!-- 년월일 -->
    <TD class='td'>
