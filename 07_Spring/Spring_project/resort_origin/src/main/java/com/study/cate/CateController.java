...
public class CateController {
		...
		@Autowired
		@Qualifier("com.study.cate.CateServiceImpl")//Qualifier 사용하는 경우 : 1.여러개가 만들어졌을 경우  2.인터페이스에서 내가 특정 그 클래스를 구현했을 경우
		...
		public ModelAndView create(CateVO cateVO) {
			...
			mav.setViewName("redirect:/cate/msg");//redirect의 경우, tiles에서 찾으면 안됨 - value값이 /cate/msg인 것을 탐색
		...
		public ModelAndView list_index_left(HttpServletRequest request) {
			...
			StringBuffer url = new StringBuffer();//비어있는 String 버퍼

			for (int index = 0; index < categrp_list.size(); index++) {//등록되어진 카테고리 그룹의 개수만큼 반복하면서 값을 얻어와 목록 생성
